...
Top level is definition-only; all non-stdlib imports are inside function
bodies so the sidecar loads in the bare skeleton env. All names are
``pdf_``-prefixed since sidecars share the kernel's ``__main__``.
...
PDF_DEFAULT_MODEL = "low-cost-default"
"""Cheap model for per-page classification. Bench (n=10 arXiv) shows a
mid-tier model at 3× cost for zero recall gain; the low-cost tier is the
right default."""
...
def pdf_guard_text(text):
    ...
    free after the first call): the sidecar AST gate only allows
    literal-constant assignments at module top level, so a module-level
    ``re.compile(...)`` binding is rejected."""
    return re.sub(
...
def pdf_prompt_blocks(instructions):
    ...
    call (prompt-injection mitigation): instructions and page text share the
...
def pdf_pages(path, mode="auto", pages=None, dpi=100, cache=True):
    ...
    if render:
        ...
        # Under.cache/ → excluded from the workspace scan → page renders do NOT
        # trigger auto image-viewing (which would spam 20 images into context
        ...
        img_dir = os.path.join(
            os.getcwd(), ".cache", "pdf-explore", f"{sha8}-{mtime}", f"dpi{int(dpi)}"
        )
    ...
    # below. When rendering is requested and pillow is
...
no recall gain from either; the bool is unreliable on image input (the model
...
query). Mitigations: block boundaries are
...
def pdf_outline(path, model=None, max_concurrency=8, force_llm=False, pages=None):
    ...
    if not force_llm:
        ...
        try:
            if toc:
                fast = [
                    {"page": int(p), "heading": str(t), "level": int(lv)}
                    for lv, t, p in toc
                    if p > 0
                ]
                if fast:
                    ...
                    try:
                        ...
                        def _norm(s):
                            return "".join(
                                c for c in _ud.normalize("NFKD", s) if c.isalnum()
                            ).lower()
    ...
    if not want_image and len(parsed) <= 150:
        ...
        body = "\n\n".join(
            ...
            + "\n"
            + (pdf_text_cap(pdf_guard_text(p["text"]), 3000) or "[no extractable text]")
            + "\n"
            + p_close
            for p in parsed
        )
            ...
            [
                {
                    ...
                    # output tokens of tool_use JSON; llm_batch disables
                    # prefill-continuation so there is no auto-continuation.
                    "max_tokens": 8192,
                    "tools": [
                        {
                            "name": "outline",
        ...
        if "error" not in r:
            entries = (r.get("tool_use") or {}).get("input", {}).get("entries") or []
            out = []
            for e in entries:
    ...
    for r in rows:
        ...
        per_page[r["page"]] = [
            h.strip() for h in hs if isinstance(h, str) and h.strip()
        ]
...
def pdf_scan(
    ...
    the doc is ≤150 text-layer pages (one low-cost call with all pages +
    ...
    :data:`PDF_DEFAULT_MODEL` (low-cost); a mid-tier model at 3× cost shows no recall
    gain on the bench.
    ...
    if strategy == "auto":
        strategy = (
            "single_call"
            if (not want_image and len(parsed) <= 150 and threshold is None)
            else "fanout"
        )
    ...
    # One llm call with ALL pages in context + a rank tool. The model compares
    ...
    if strategy == "single_call" and not want_image:
        ...
        r = pdf_sdk().llm(
            [
                {
                    "prompt": (
                        f"{hdr}{q_open}{pdf_guard_text(query)}{q_close}" f"\n\n{body}"
                    ),
                    "model": model,
        ...
        ranked = (r.get("tool_use") or {}).get("input", {}).get("top_pages") or []
        ...
        if not ranked and r.get("stop_reason") == "max_tokens":
            return {
                ...
                "error": (
                    "pdf_scan: ranking tool_use truncated at "
        ...
        for pg in ranked:
            ...
            # the whole result (single-call page ranking).
            try:
                n = int(pg)
